 ...
 *
 * $Id: ATState.java,v 1.1 2002/08/05 00:06:41 tgautier Exp $
 */
package JavaTron;
...
public class ATState
{
  ...
  // define playing states (no enums in java)
  ...
  //public static final ATState SEARCHING = newState("Searching");
  ...
  static {
    ...
    g.fillOval((int) (img.getWidth()*.20), (int) (img.getHeight()*.20), 
               (int) (img.getWidth()*.60), (int) (img.getHeight()*.60));
    
    pausedImage = img = getNewImage();
    g = img.getGraphics();
    g.setColor(Color.green);
    g.fillRect((int) (img.getWidth()*.30), (int) (img.getHeight()*.20),
               (int) (img.getWidth()*.15), (int) (img.getHeight()*.60)); 
    
    g.fillRect((int) (img.getWidth()*.55), (int) (img.getHeight()*.20),
               (int) (img.getWidth()*.15), (int) (img.getHeight()*.60)); 
  }

  private static BufferedImage getNewImage()
  {
    BufferedImage image = new BufferedImage(48, 
                                            36, 
  ...
  {
    return (this == ATState.PAUSED ||
            this == ATState.PLAYING ||
            this == ATState.BUFFERING ||
            this == ATState.BUFFERED ||
            this == ATState.OPENING);
  }
