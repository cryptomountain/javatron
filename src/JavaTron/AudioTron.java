 ...
 *
 * $Id: AudioTron.java,v 1.1 2002/08/05 00:06:41 tgautier Exp $
 */
package JavaTron;
...
public class AudioTron
{
  ...
  // the get connection, so it can be terminated
  ...
  public abstract static class AbstractParser implements Parser
  {
    ...
    public void end(boolean error) { }
	@Override
    public boolean parse(String content) { return true; }
  }
   ...
   */
  public AudioTron(String server) { this(server, null); }
  /**
  ...
  public String getServer() { return Configuration.getProperty(Configuration.KEY_SERVER); }
  public String getUsername() { return Configuration.getProperty(Configuration.KEY_USERNAME); }
  public String getPassword() { 
	  return( new String(JTP.decrypt(Configuration.getProperty(Configuration.KEY_PASSWORD))) );
  }
  public String getBaseM3U() {return Configuration.getProperty(Configuration.KEY_BASE_M3U_FILE); }
  public String getPlaylistDefaultPath(){ return Configuration.getProperty(Configuration.KEY_DEFAULT_PLAYLIST_PATH); }
  ...
  */
  ...
  {
    GetCommand command = new GetCommand("/clockctrl.asp",
                                        null,
                                        parser,
                                        CLOCK);
  ...
  public void setNap(int minutes, boolean softWake)
  {
    ...
    GetCommand command = new GetCommand("/goform/webSetNapForm",
                                        commandArgs,
                                        null,
                                        NAP);
    addCommand(command, 0);
                       ...
                       int volume)
  {
    ...
    if (enabled) {
      ...
      if (days.get(0)) {
        commandArgs.add("alarmmon"); commandArgs.add("ON");
      }
      if (days.get(1)) {
        commandArgs.add("alarmtue"); commandArgs.add("ON");
      }
      if (days.get(2)) {
        commandArgs.add("alarmwed"); commandArgs.add("ON");
      }
      if (days.get(3)) {
        commandArgs.add("alarmthu"); commandArgs.add("ON");
      }
      if (days.get(4)) {
        commandArgs.add("alarmfri"); commandArgs.add("ON");
      }
      if (days.get(5)) {
        commandArgs.add("alarmsat"); commandArgs.add("ON");
      }
      if (days.get(6)) {
        commandArgs.add("alarmsun"); commandArgs.add("ON");
      }
    ...
    GetCommand command = new GetCommand("/goform/webSetAlarmForm",
                                         commandArgs,
                                         null,
                                         ALARM);
  ...
  public void getGlobalInfo(Parser parser)
  {
    ...
    GetCommand command = new GetCommand("/apigetinfo.asp",
                                        commandArgs,
                                        parser,
                                        GLOBAL);
  ...
  public void getTOC(String share, Parser parser)
  {
    ...
    GetCommand command = new GetCommand("/apidumptoc.asp",
                                        commandArgs,
                                        parser,
                                        TOC);
    addCommand(command, 0);
  ...
  public void setTimeFromTimeServer(String timeServer, Parser parser)
  {
    ...
    GetCommand command = new GetCommand("/goform/webSetTimeNTPForm",
                                        commandArgs,
                                        parser,
                                        SET_CLOCK);
  ...
  public void getFilteredInfo(String filter,
                              String filterValue,
                              Parser parser)
  {    
  ...
  public void queueFile(Playable playable)
  {
    ...
    GetCommand command = new GetCommand("/apiqfile.asp",
                                        commandArgs,
                                        null);
    addCommand(command, 0);
  }
   ...
   * request. Probably should use a Vector for consistency, but I'm
  ...
  public void dequeueFiles(HashMap args) {
    ...
    PostCommand command = new PostCommand("/goform/webQuePlayForm",
                                        commandArgs,
                                        null,
										DELQUE);
    if(showExtra){System.out.println("Dequeueing ....");}
	addCommand(command, 0);
  }
  /**
   * Tell the audiotron to play a file (same as queueFile but clears
   ...
   */
	public void addToLibrary(String songLoc){
		...
		GetCommand command = new GetCommand("/apiaddfile.asp",
											commandArgs,
											null,
											ADDNEW);
		if(showExtra){System.out.println("Adding "+songLoc);}
		//addCommand(command, 0);
	}

  	/**
	 * Method to add a new share
	 *
	 * @param shareURL
	 */
	public void add_new_share(String shareURL){
	...
	*/
	public void search(){
	  ...
	  PostCommand command = new PostCommand("/goform/CheckNewFilesForm",
											commandArgs,
											null,
											SEARCH);
	  if(showExtra){System.out.println("Searching for new Files.");}
	  addCommand(command, 0);

	}

  /**
   * Tell's the audiotron to re-read the radio.txt file
   *
   */
  public void checkRadioText(){
	  ...
	  commandArgs.add("Search");
	  PostCommand command = new PostCommand("/goform/CheckNewStationsForm",
											commandArgs,
											null,
											SEARCH);
	  if(showExtra){System.out.println("Searching for new internet stations.");
	  addCommand(command,0);
	  }
 ...
 public void setXPAuth(String username,String password){
	...
	PostCommand command=new PostCommand("/goform/NTForm",
										commandArgs,
										null);
	addCommand(command,0);
 }
 ...
 */
 public void setWin98Auth(String password){
	 ...
	 PostCommand command=new PostCommand("goform/ShareForm",
										commandArgs,
										null);
	addCommand(command,0);
  ...
  public void getPage(String address,Vector commandArgs, JTP.HtmlParser parser){
	  ...
	  GetCommand command = new GetCommand(address,
											commandArgs,
											parser,
											INFO);
	  if(showExtra){System.out.println("Getting "+ address);}
	  addCommand(command,0);
  }
  /**
   * Issue a command
  ...
  {     
    ...
    GetCommand command = new GetCommand("/apicmd.asp",
                                        commandArgs,
                                        null);
    addCommand(command, 0);
  }
  ...
  protected class GetCommand extends Command
  {
    ...
    */
    ...
    {
      super(status);
      address = address_; args = args_; parser = parser_;
    }
  ...
  protected String post(String address, Vector args, Parser parser) throws IOException
  {
	...
	try {
	  ...
	  // Debug
	  if(showAuth){ System.out.println("POST: AUTH: "+ auth);}
  ...
  throws MalformedURLException, IOException
  {
    ...
    // setup the url and open a connection to it
	...
	conn.setRequestProperty("Authorization",
                            "Basic " +
							B64Encode(auth.getBytes())
							);
	// Debug
   if(showAuth){ System.out.println("AUTH: "+ auth);}
    return conn;
  }
  ...
  protected String B64Encode(byte[] s){
	  ...
	  // Debug
	  if(showExtra){System.out.println("Encode: "+h);}
  ...
  */
  protected int getCommandStatus() { return commandThread.getCommandStatus(); }
  ...
  */
  ...
  private class CommandThread 
	  implements Runnable, PriorityQueue.ElementInsertionListener
  {
  ...
  {
    int status;    
    public Command(int status_) { status = status_; }    
